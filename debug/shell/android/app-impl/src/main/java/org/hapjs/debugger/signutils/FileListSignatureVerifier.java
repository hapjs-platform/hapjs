...
public class FileListSignatureVerifier {
    ...
    public static void verify(File apkFile, SignatureVerifier.SignatureInfo fileListSignature, Certificate certificate)
            throws SignatureVerifier.SignatureNotFoundException, SecurityException, IOException {
        ZipFile zipFile = new ZipFile(apkFile);

        ByteBuffer byteBuffer = SignatureVerifier.getLengthPrefixedSlice(fileListSignature.signatureBlock);
        ...
        byte[] bestSigAlgorithmSignatureBytes = SignatureVerifier.readLengthPrefixedByteArray(signature);
        ...
        while (digests.hasRemaining()) {
            try {
                ...
                throw new SecurityException(
                        "Failed to parse digests record",
                        e);
            }
        }
        // verify every entry
    ...
    private static void verifyEveryEntry(Map<Integer, List<byte[]>> digestsMap, ZipFile zipFile, int bestSigAlgorithm)
            throws IOException, DigestException {
        int digestAlgorithm = SignatureVerifier.getSignatureAlgorithmContentDigestAlgorithm(bestSigAlgorithm);
        ...
        int digestOutputSizeBytes = SignatureVerifier.getContentDigestAlgorithmOutputSizeBytes(digestAlgorithm);
