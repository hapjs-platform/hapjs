 ...
 */
public abstract class BaseDatabaseDriver<T> {
    ...
    /**
     * Access a stable list of objects that describe the databases made available by this driver. The
     * list of returned objects must not change on each invocation as this will cause a memory leak
     * when assigning unique identifiers for the objects to remote peers.
     */
    public abstract List<T> getDatabaseNames();

    /**
     * Get or create a list of table names given a previously returned database descriptor instance
     * from {@link #getDatabaseNames()}.
     */
    public abstract List<String> getTableNames(T database);

    public abstract Database.ExecuteSQLResponse executeSQL(
            T database, String query, ExecuteResultHandler<Database.ExecuteSQLResponse> handler)
