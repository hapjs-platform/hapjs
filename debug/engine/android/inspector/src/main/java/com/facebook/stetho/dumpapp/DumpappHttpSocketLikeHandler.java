...
/**
 * Provides a legacy hook to support old dumpapp scripts which have not been updated to the new
 * binary protocol. Since the framing protocol is still compatible this can be easily supported to
 * reduce upgrade friction since so many developers forgot to copy the {@code dumpapp} script on
 * upgrade.
 *
 * <p>{@code stdin} is not supported with this legacy mode. Streaming output is also not supported.
 *
 * <p>This script sends a warning to stderr informing the developer of its deprecated nature.
 */
@Deprecated
                ...
                framer
                        ...
                        .println(
                                "*** "
                                        + warningPrefix
                                        +
                                        ": Using legacy HTTP protocol; update dumpapp script! ***");

                if (getMethod) {
                    DumpappSocketLikeHandler
                            .dump(mDumper, framer, argv.toArray(new String[argv.size()]));
                } else {
