...
/**
 * Socket handler which is designed to detect a difference in protocol signatures very early on in
 * the connection to figure out which real handler to route to. This is used for performance and
 * backwards compatibility reasons to maintain Stetho having just one actual socket connection
 * despite dumpapp and DevTools now diverging in protocol.
 *
 * <p>Note this trick is only possible if the protocol requires that the client initiate the
...
public class ProtocolDetectingSocketHandler extends SecureSocketHandler {
    ...
    @Override
    protected void onSecured(AbsSocket socket) throws IOException {
        LeakyBufferedInputStream leakyIn =
                new LeakyBufferedInputStream(socket.getInputStream(), SENSING_BUFFER_SIZE);
    ...
    public static class HapServiceMatcher implements MagicMatcher {
        private final String[] urls;

        public HapServiceMatcher(String[] us) {
            urls = us;
        }
        ...
        public boolean matches(InputStream in) throws IOException {
            for (String url : urls) {
                byte[] mMagic = ("GET /" + url).getBytes();
                byte[] buf = new byte[mMagic.length];
                int n = in.read(buf);
                if (n == buf.length && Arrays.equals(buf, mMagic)) {
                    return true;
                }
            }
            return false;
