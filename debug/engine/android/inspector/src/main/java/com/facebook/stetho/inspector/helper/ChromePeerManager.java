...
/**
 * Interface glue that allows a particular domain to manage the enabled peers. The way the WebKit
 * inspector protocol works is that each functionality domain has an enable/disable JSON-RPC method
 * call which alerts the server (that's us) that we can now begin sending local events to the peer
 * to have them appear in the inspector UI. This class simplifies managing those enabled peers for
 * each functionality domain.
 */
public class ChromePeerManager {
    ...
    /**
     * This should be set to null anytime mReceivingPeers is changed. It should always be retrieved by
     * calling getReceivingPeersSnapshot().
     */
    @GuardedBy("this")
    ...
    /**
     * Set a listener which can receive notifications of unique registration event (see {@link
     * #addPeer} and {@link #removePeer}).
     *
     * @param listener
     */
    ...
    private synchronized JsonRpcPeer[] getReceivingPeersSnapshot() {
        if (mReceivingPeersSnapshot == null) {
            mReceivingPeersSnapshot =
                    mReceivingPeers.keySet().toArray(new JsonRpcPeer[mReceivingPeers.size()]);
        }
    ...
    private void sendMessageToPeers(
            String method, Object params, @Nullable PendingRequestCallback callback) {
