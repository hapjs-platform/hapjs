...
public class SqliteDatabaseDriver
     ...
     * @deprecated use {@link SqliteDatabaseDriver#SqliteDatabaseDriver(Context, String,
     * DatabaseFilesProvider, DatabaseConnectionProvider)}
     */
    @Deprecated
     ...
     * that this only removes the database if it is true that it shadows another database lacking the
     * uninteresting suffix.
     *
    ...
    static List<File> tidyDatabaseList(List<File> databaseFiles) {
        ...
        for (File databaseFile : databaseFiles) {
            ...
            if (sansSuffix.equals(databaseFilename)
                    || !originalAsSet.contains(new File(sansSuffix))) {
    ...
    private <T> T executeUpdateDelete(
            SQLiteDatabase database, String query, ExecuteResultHandler<T> handler) {
        // INSPECTOR MOD BEGIN:
    ...
    private SQLiteDatabase openDatabase(SqliteDatabaseDescriptor databaseDesc)
            throws SQLiteException {
