...
/**
 * Abstract class for pretty printer factory that asynchronously downloads schema needed for pretty
...
public abstract class DownloadingAsyncPrettyPrinterFactory implements AsyncPrettyPrinterFactory {
    ...
    private static AsyncPrettyPrinter getErrorAsyncPrettyPrinter(
        ...
        return new AsyncPrettyPrinter() {
            ...
            public void printTo(PrintWriter output, InputStream payload) throws IOException {
                String errorMessage =
                        "[Failed to parse header: " + headerName + " : " + headerValue + " ]";
                    ...
                    try {
                        ...
                        try {
                            ...
                            if (IOException.class.isInstance(cause)) {
                                doErrorPrint(
                                        output, payload,
                                        "Cannot successfully download schema: " + e.getMessage());
                                return;
                            } else {
                                throw e;
                            }
     ...
     * @param headerValue header value which contains the URI for the schema data needed to pretty
     *                    print the response body
     * @return MatchResult that has the schema uri and the type of prettified result. Null if there is
     * no correct header match.
     */
    @Nullable
    protected abstract MatchResult matchAndParseHeader(String headerName, String headerValue);

    /**
     * Note that the IOException thrown by this method will be propagated all the way up and yield an
     * error to the chrome devtools
     */
