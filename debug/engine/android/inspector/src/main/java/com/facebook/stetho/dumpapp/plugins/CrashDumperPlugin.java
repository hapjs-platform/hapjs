...
public class CrashDumperPlugin implements DumperPlugin {
    ...
    private void doUsage(PrintStream out) {
        ...
        out.println(
                "    <Throwable>: Throwable class to use (default: " + OPTION_THROW_DEFAULT + ")");
        out.println();
        out.println(
                cmdName + " kill: Send a signal to this process (simulates the low memory killer)");
        out.println(
                "    <SIGNAL>: Either signal name or number to send (default: "
                        + OPTION_KILL_DEFAULT
                        + ")");
    ...
    private void doKill(DumperContext dumpContext, Iterator<String> argsIter) throws DumpException {
        ...
        try {
            Process kill =
                    new ProcessBuilder()
                            .command("/system/bin/kill", "-" + signal,
