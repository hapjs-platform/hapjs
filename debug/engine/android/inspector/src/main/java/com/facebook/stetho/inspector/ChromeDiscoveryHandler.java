 ...
 * "one of them". Note that we are being discovered automatically by the name of our socket as
 * defined in {@link LocalSocketHttpServer}. After discovery, we're required to provide some context
 * on how exactly to display and inspect what we have.
 */
public class ChromeDiscoveryHandler implements HttpHandler {
    ...
    public static void setSuccessfulResponse(LightHttpResponse response, LightHttpBody body) {
    ...
    private String makeTitle() {
        ...
        int colonIndex = TextUtils.isEmpty(processName) ? -1 : processName.indexOf(':');
        if (colonIndex >= 0) {
    ...
    private void handleActivate(LightHttpResponse response) {
        ...
        setSuccessfulResponse(
                response, LightHttpBody.create("Target activation ignored\n", "text/plain"));
    }
