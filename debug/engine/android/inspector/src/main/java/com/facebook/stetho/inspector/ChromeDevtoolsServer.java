...
/**
 * Implements a limited version of the Chrome Debugger WebSocket protocol (using JSON-RPC 2.0). The
 * most up-to-date documentation can be found in the Blink source code: <a
 * href="https://code.google.com/p/chromium/codesearch#chromium/src/third_party/WebKit/Source/devtools/protocol.json&q=protocol.json&sq=package:chromium&type=cs">protocol.json</a>
 */
public class ChromeDevtoolsServer implements SimpleEndpoint {
    ...
    private final Map<SimpleSession, JsonRpcPeer> mPeers =
            Collections.synchronizedMap(new HashMap<SimpleSession, JsonRpcPeer>());
    ...
    public void onMessage(SimpleSession session, String message) {
        ...
        DebuggerLogUtil.logBreadcrumb("ChromeDevtoolsServer.onMessage, message=" + message);
        try {
    ...
    private void handleRemoteMessage(JsonRpcPeer peer, String message)
        ...
        if (messageNode.has("method")) {
            ...
            handleRemoteRequest(peer, messageNode, message);
    ...
    private void handleRemoteRequest(JsonRpcPeer peer, JSONObject requestNode, String message)
        ...
        try {
            ...
            // INSPECTOR ADD BEGIN:
            if (e.getErrorMessage().code.getProtocolValue()
                    == JsonRpcError.ErrorCode.METHOD_NOT_FOUND.getProtocolValue()) {
                org.hapjs.inspector.V8Inspector.getInstance().handleMessage(peer, message);
                return;
            }
            // END
