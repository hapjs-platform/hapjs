...
public interface SimpleEndpoint {
    ...
    /**
     * Invoked when a binary message is received from the peer. May have spanned multiple WebSocket
     * packets.
     *
     * @param session    Unique handle for this session.
     * @param message    Complete payload data.
    ...
    /**
     * Invoked when a remote peer closed the WebSocket session or if {@link SimpleSession#close} is
     * invoked on our side.
     *
     * @param session           Unique handle for this session.
    ...
    /**
     * Invoked when errors occur out of the normal band of the WebSocket protocol. This is intended
     * for debug purposes and is generally not actionable. The {@link #onClose} method will still be
     * invoked in all cases, making it reasonable to simply log in this method.
     *
     * @param session Unique handle for this session.
