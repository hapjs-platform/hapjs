...
/**
 * Optimization designed to allow us to lazily construct/configure the true Stetho server only after
 * the first caller connects. This gives us much more wiggle room to have performance impact in the
 * set up path that only applies when Stetho is _used_, not simply enabled.
 */
public class LazySocketHandler implements SocketHandler {
    ...
    @Override
    public void onAccepted(AbsSocket socket) throws IOException {
