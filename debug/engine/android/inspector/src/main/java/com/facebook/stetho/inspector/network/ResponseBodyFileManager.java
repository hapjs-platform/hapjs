...
public class ResponseBodyFileManager {
    ...
    private final Map<String, AsyncPrettyPrinter> mRequestIdMap =
            Collections.synchronizedMap(new HashMap<String, AsyncPrettyPrinter>());
    ...
    private String prettyPrintContentWithTimeOut(
            AsyncPrettyPrinter asyncPrettyPrinter, InputStream in) throws IOException {
        AsyncPrettyPrintingCallable prettyPrintingCallable =
                new AsyncPrettyPrintingCallable(in, asyncPrettyPrinter);
        ...
        try {
            ...
            future.cancel(true);
            return "Time out after "
                    + PRETTY_PRINT_TIMEOUT_SEC
    ...
    /**
     * Associates an asynchronous pretty printer with a response request id The pretty printer will be
     * used to pretty print the response body that has the particular request id
     *
     * @param requestId          Unique identifier for the response as per {@link
     *                           NetworkEventReporter.InspectorResponse#requestId()}
    ...
    public void associateAsyncPrettyPrinterWithId(
            String requestId, AsyncPrettyPrinter asyncPrettyPrinter) {
        if (mRequestIdMap.put(requestId, asyncPrettyPrinter) != null) {
            throw new IllegalArgumentException(
                    "cannot associate different " + "pretty printers with the same request id: "
                            + requestId);
        }
    }
