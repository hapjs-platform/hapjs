...
/**
 * A LayoutManager that lays out children in a staggered grid formation. It supports horizontal &
 * vertical layout as well as an ability to layout children in reverse.
 *
 * <p>Staggered grids are likely to have gaps at the edges of the layout. To avoid these gaps,
 * HapStaggeredGridLayoutManager can offset spans independently or move items between spans. You can
 * control this behavior via {@link #setGapStrategy(int)}.
 */
public class HapStaggeredGridLayoutManager extends RecyclerView.LayoutManager
     ...
     * check if there are gaps in the because of full span items. If it finds, it will re-layout and
     * move items to correct positions with animations.
     *
     * <p>For example, if LayoutManager ends up with the following layout due to adapter changes:
     *
     * <pre>
     * AAA
     * _BC
     ...
     * </pre>
     *
     * <p>It will animate to the following state:
     *
     * <pre>
     * AAA
     * BC_
    ...
    private static final String TAG = "HapStaggeredGLManager";
    /**
     * While trying to find next view to focus, LayoutManager will not try to scroll more than this
     * factor times the total space of the list. If layout is vertical, total space is the height
     * minus padding, if layout is horizontal, total space is the width minus padding.
     */
    ...
    Span[] mSpans;
    /**
     * Primary orientation is the layout's orientation, secondary orientation is the orientation for
    ...
    /**
     * Keeps the mapping between the adapter positions and spans. This is necessary to provide a
     * consistent experience when user scrolls the list.
     */
    ...
    /**
     * If a full span item is invalid / or created in reverse direction; it may create gaps in the UI.
     * While laying out, if such case is detected, we set this flag.
     *
     * <p>After scrolling stops, we check this flag and if it is set, re-layout.
     */
     ...
     * Defaults to single column and vertical.
     */
    @SuppressWarnings("unused")
    public HapStaggeredGridLayoutManager(
    ...
    /**
     * Creates a HapStaggeredGridLayoutManager with given parameters.
     *
     * @param spanCount   If orientation is vertical, spanCount is number of columns. If orientation is
     *                    horizontal, spanCount is number of rows.
     * @param orientation {@link #VERTICAL} or {@link #HORIZONTAL}
     */
    public HapStaggeredGridLayoutManager(int spanCount, int orientation) {
     ...
     *
     * <p>When a full span item is laid out in reverse direction, it sets a flag which we check when
    ...
    boolean checkForGaps() {
        ...
        int invalidGapDir =
                mShouldReverseLayout ? LayoutState.LAYOUT_START : LayoutState.LAYOUT_END;
     ...
     *
     * <p>Intermediate gaps created by full span items are tracked via mLaidOutInvalidFullSpan field.
     */
    View hasGapsToFix() {
    ...
    /**
     * Returns the current gap handling strategy for HapStaggeredGridLayoutManager.
     *
     * <p>Staggered grid may have gaps in the layout due to changes in the adapter. To avoid gaps,
     * HapStaggeredGridLayoutManager provides 2 options. Check {@link #GAP_HANDLING_NONE} and {@link
     * #GAP_HANDLING_MOVE_ITEMS_BETWEEN_SPANS} for details.
     *
     * <p>By default, HapStaggeredGridLayoutManager uses {@link
    ...
    /**
     * Sets the gap handling strategy for HapStaggeredGridLayoutManager. If the gapStrategy parameter
    ...
    /**
     * Returns the number of spans laid out by HapStaggeredGridLayoutManager.
     *
    ...
    /**
     * Sets the number of spans for the layout. This will invalidate all of the span assignments for
     * Views.
     *
     * <p>Calling this method will automatically result in a new layout request unless the spanCount
    ...
    /**
     * For consistency, HapStaggeredGridLayoutManager keeps a mapping between spans and items.
     *
     * <p>If you need to cancel current assignments, you can call this method which will clear all
    ...
    /**
     * Calculates the views' layout order. (e.g. from end to start or start to end) RTL layout support
     ...
     *
     * <p>Not that this value is not affected by RecyclerView's layout direction.
     *
     ...
     *
     * <p>For vertical layout, if it is set to <code>true</code>, first item will be at the bottom of
     * the list.
     *
    ...
    @Override
    public void setMeasuredDimension(Rect childrenBounds, int widthSpec, int heightSpec) {
        ...
        if (mOrientation == VERTICAL) {
            ...
            height = chooseSize(heightSpec, usedHeight, getMinimumHeight());
            width = chooseSize(widthSpec, mSizePerSpan * mSpanCount + horizontalPadding,
                    getMinimumWidth());
        } else {
            final int usedWidth = childrenBounds.width() + horizontalPadding;
            width = chooseSize(widthSpec, usedWidth, getMinimumWidth());
            height = chooseSize(heightSpec, mSizePerSpan * mSpanCount + verticalPadding,
                    getMinimumHeight());
        }
    ...
    private void onLayoutChildren(
            RecyclerView.Recycler recycler, RecyclerView.State state, boolean shouldCheckForGaps) {
    ...
    void updateMeasureSpecs(int totalSpace) {
        ...
        mFullSizeSpec =
                View.MeasureSpec.makeMeasureSpec(totalSpace, mSecondaryOrientation.getMode());
    }
     ...
     *
     * <p>Note that, this value is not affected by layout orientation or item order traversal. ({@link
     * #setReverseLayout(boolean)}). Views are sorted by their positions in the adapter, not in the
     * layout.
     *
     * <p>If RecyclerView has item decorators, they will be considered in calculations as well.
     *
     * <p>HapStaggeredGridLayoutManager may pre-cache some views that are not necessarily visible.
     ...
     * @param into An array to put the results into. If you don't provide any, LayoutManager will
     *             create a new one.
     * @return The adapter position of the first fully visible item in each span. If a span does not
     ...
     * @param into An array to put the results into. If you don't provide any, LayoutManager will
     *             create a new one.
     * @return The adapter position of the last visible item in each span. If a span does not have any
     * items, {@link RecyclerView#NO_POSITION} is returned for that span.
     * @see #findLastCompletelyVisibleItemPositions(int[])
     * @see #findFirstVisibleItemPositions(int[])
     */
    ...
    private void measureChildWithDecorationsAndMargin(
            View child, LayoutParams lp, boolean alreadyMeasured) {
        if (lp.mFullSpan) {
            if (mOrientation == VERTICAL) {
            ...
            if (mOrientation == VERTICAL) {
                ...
                measureChildWithDecorationsAndMargin(
                        child,
                        getChildMeasureSpec(
                                mSizePerSpan, getWidthMode(), 0, lp.leftMargin + lp.rightMargin,
                                lp.width, false),
                        getChildMeasureSpec(
                                getHeight(),
                                getHeightMode(),
                ...
                measureChildWithDecorationsAndMargin(
                        ...
                        getChildMeasureSpec(
                                ...
                                true),
                        getChildMeasureSpec(
                                mSizePerSpan, getHeightMode(), 0, lp.bottomMargin + lp.topMargin,
                                lp.height, false),
                        alreadyMeasured);
            }
        }
    ...
    private void measureChildWithDecorationsAndMargin(
            View child, int widthSpec, int heightSpec, boolean alreadyMeasured) {
    ...
    private int getChildMeasureSpec(
        ...
        if (canScroll) {
            if (childDimension >= 0) {
                resultSize = childDimension + childMargin;
            ...
            if (childDimension >= 0) {
                ...
                resultSize = size;
                if (parentMode == View.MeasureSpec.AT_MOST
                        || parentMode == View.MeasureSpec.EXACTLY) {
    ...
    public Parcelable onSaveInstanceState() {
        ...
        if (getChildCount() > 0) {
            state.mAnchorPosition =
                    mLastLayoutFromEnd ? getLastChildPosition() : getFirstChildPosition();
        ...
        if (mOrientation == HORIZONTAL) {
            info.setCollectionItemInfo(
                    AccessibilityNodeInfoCompat.CollectionItemInfoCompat.obtain(
                            sglp.getSpanIndex(), sglp.mFullSpan ? mSpanCount : 1, -1, -1, false,
                            false));
        } else { // VERTICAL
            info.setCollectionItemInfo(
                    AccessibilityNodeInfoCompat.CollectionItemInfoCompat.obtain(
                            -1, -1, sglp.getSpanIndex(), sglp.mFullSpan ? mSpanCount : 1, false,
                            false));
        }
    }
    ...
    /**
     * This is for internal use. Not necessarily the child closest to start but the first child we
     * find that matches the criteria. This method does not do any sorting based on child's start
     * coordinate, instead, it uses children order.
     */
    ...
    /**
     * This is for internal use. Not necessarily the child closest to bottom but the first child we
     * find that matches the criteria. This method does not do any sorting based on child's end
     * coordinate, instead, it uses children order.
     */
    ...
    private void updateLayoutState(int anchorPosition, RecyclerView.State state) {
        ...
        mLayoutState.mInfinite =
                mPrimaryOrientation.getMode() == View.MeasureSpec.UNSPECIFIED
    ...
    private void setLayoutStateDirection(int direction) {
        ...
        mLayoutState.mItemDirection =
                (mShouldReverseLayout == (direction == LayoutState.LAYOUT_START))
    ...
    private int calculateScrollDirectionForPosition(int position) {
        ...
        return position < firstChildPos != mShouldReverseLayout
     ...
     *
     * <p>Note that scroll position change will not be reflected until the next layout call.
     *
     * <p>If you are just trying to make a position visible, use {@link #scrollToPosition(int)}.
     *
    ...
    public void collectAdjacentPrefetchPositions(
            int dx, int dy, RecyclerView.State state,
        ...
        for (int i = 0; i < mSpanCount; i++) {
            ...
            int distance =
                    mLayoutState.mItemDirection == LayoutState.LAYOUT_START
                            ? mLayoutState.mStartLine
                            - mSpans[i].getStartLine(mLayoutState.mStartLine)
        ...
        for (int i = 0; i < itemPrefetchCount && mLayoutState.hasMore(state); i++) {
            layoutPrefetchRegistry
                    .addPosition(mLayoutState.mCurrentPosition, mPrefetchDistances[i]);
    ...
    /**
     * Sets the orientation of the layout. HapStaggeredGridLayoutManager will do its best to keep
    ...
    /**
     * LayoutParams used by HapStaggeredGridLayoutManager.
     *
     * <p>Note that if the orientation is {@link #VERTICAL}, the width parameter is ignored and if the
    ...
    public static class LayoutParams extends RecyclerView.LayoutParams {
        ...
        /**
         * When set to true, the item will layout using all span area. That means, if orientation is
         * vertical, the view will have full width; if orientation is horizontal, the view will have
         * full height.
         *
         ...
         *
         * @return The Span index of the View. If View is not yet assigned to any span, returns {@link
         * #INVALID_SPAN_ID}.
         */
    ...
    /**
     * An array of mappings from adapter position to span. This only grows when a write happens and it
     * grows up to the size of the adapter.
     */
    static class LazySpanLookup {
        ...
        /**
         * Returns when invalidation should end. e.g. hitting a full span position. Returned position
    ...
    class Span {
        ...
        /**
         * Returns the first view within this span that is partially or fully visible. Partially visible
         * refers to a view that overlaps but is not fully contained within RV's padded bounded area.
         * This view returned can be defined to have an area of overlap strictly greater than zero if
         * acceptEndPointInclusion is false. If true, the view's endpoint inclusion is enough to
         * consider it partially visible. The latter case can then refer to an out-of-bounds view
         * positioned right at the top (or bottom) boundaries of RV's padded area. This is used e.g.
         * inside {@link #onFocusSearchFailed(View, int, RecyclerView.Recycler, RecyclerView.State)} for
         ...
         * @param completelyVisible       True if we have to only consider completely visible views, false
         *                                otherwise.
         * @param acceptCompletelyVisible True if we can consider both partially or fully visible views,
         *                                false, if only a partially visible child should be returned.
         * @param acceptEndPointInclusion If the view's endpoint intersection with RV's padded bounded
         *                                area is enough to consider it partially visible, false otherwise
        ...
        int findOnePartiallyOrCompletelyVisibleChild(
            ...
            for (int i = fromIndex; i != toIndex; i += next) {
                ...
                boolean childStartInclusion =
                        acceptEndPointInclusion ? (childStart <= end) : (childStart < end);
                boolean childEndInclusion =
                        acceptEndPointInclusion ? (childEnd >= start) : (childEnd > start);
        ...
        int findOneVisibleChild(int fromIndex, int toIndex, boolean completelyVisible) {
            return findOnePartiallyOrCompletelyVisibleChild(
                    fromIndex, toIndex, completelyVisible, true, false);
        }

        int findOnePartiallyVisibleChild(int fromIndex, int toIndex,
                                         boolean acceptEndPointInclusion) {
            return findOnePartiallyOrCompletelyVisibleChild(
                    fromIndex, toIndex, false, false, acceptEndPointInclusion);
        }
