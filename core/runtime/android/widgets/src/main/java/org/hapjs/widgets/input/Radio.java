...
@WidgetAnnotation(
        ...
        types = {@TypeAnnotation(name = Radio.TYPE_RADIO)},
        methods = {
...
public class Radio extends Button implements SingleChoice {
    ...
    private void initOnCheckedListener(FlexRadioButton radioButton) {
        if (radioButton == null) {
            return;
        }
        radioButton.setOnCheckedChangeListener(
                new CompoundButton.OnCheckedChangeListener() {
                    ...
                    public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                        ...
                        if (isChecked) {
                            getRootComponent().handleSingleChoice(mName, Radio.this);
                        }

                        if (isCheckEventRegistered && isChecked) {
                            Map<String, Object> params = new HashMap();
                            params.put("name", mName);
                            params.put("value", mValue);
                            mCallback.onJsEventCallback(
                                    getPageId(), mRef, Attributes.Event.CHANGE, Radio.this, params,
                                    null);
                        }
                    }
                });
    ...
    public void setChecked(boolean checked) {
        ...
        ((FlexRadioButton) mHost).setChecked(checked);
    }
    ...
    public void destroy() {
        super.destroy();
        if (mHost != null) {
            ((FlexRadioButton) mHost).setOnCheckedChangeListener(null);
        }
    }
}
