 ...
 *       {@link Number#doubleValue() doubleValue}. Strings that can be coerced using {@link
 *       Double#valueOf(String)} will be.
 *   <li>When the requested type is an int, other {@link Number} types will be coerced using {@link
 *       Number#intValue() intValue}. Strings that can be coerced using {@link
 *       Double#valueOf(String)} will be, and then cast to int.
 *   <li><a name="lossy">When the requested type is a long, other {@link Number} types will be
 *       coerced using {@link Number#longValue() longValue}. Strings that can be coerced using
 *       {@link Double#valueOf(String)} will be, and then cast to long. This two-step conversion is
 *       lossy for very large values. For example, the string "9223372036854775806" yields the long
 *       9223372036854775807.</a>
 *   <li>When the requested type is a String, other non-null values will be coerced using {@link
 *       String#valueOf(Object)}. Although null cannot be coerced, the sentinel value {@link
 *       JSONObject#NULL} is coerced to the string "null".
 * </ul>
 *
 * <p>This class can look up both mandatory and optional values:
 *
 * <ul>
 *   <li>Use <code>get<i>Type</i>()</code> to retrieve a mandatory value. This fails with a {@code
 ...
 *       user-supplied default if the requested name has no value or if the value cannot be coerced
 *       to the requested type.
 * </ul>
 *
 * <p><strong>Warning:</strong> this class represents null in two incompatible ways: the standard
 * Java {@code null} reference, and the sentinel value {@link JSONObject#NULL}. In particular,
 * calling {@code put(name, null)} removes the named entry from the object but {@code put(name,
 * JSONObject.NULL)} stores an entry whose value is {@code JSONObject.NULL}.
 *
 * <p>Instances of this class are not thread safe. Although this class is nonfinal, it was not
 * designed for inheritance and should not be subclassed. In particular, self-use by overrideable
...
public class JSONObject {
     ...
     * </ul>
     *
     * <p>This value violates the general contract of {@link Object#equals} by returning true when
    ...
    private static final String TAG = "JSONObejct";
    ...
    public static String numberToString(Number number) throws JSONException {
        ...
        if (FloatUtil.doublesEqual(doubleValue, (double) longValue)) {
     ...
     * object is a primitive wrapper type or {@code String}, returns the object. Otherwise if the
     * object is from a {@code java} package, returns the result of {@code toString}. If wrapping
     * fails, returns null.
     */
     ...
     * a time.
     *
     * <p>Note that {@code append(String, Object)} provides better semantics. In particular, the
     * mapping for {@code name} will <b>always</b> be a {@link JSONArray}. Using {@code accumulate}
     * will result in either a {@link JSONArray} or a mapping whose type is the type of {@code value}
    ...
    /**
     * Appends values to the array mapped to {@code name}. A new {@link JSONArray} mapping for {@code
     * name} will be inserted if no mapping exists. If the existing mapping for {@code name} is not a
     ...
     * long, or throws otherwise. Note that JSON represents numbers as doubles, so this is <a
     * href="#lossy">lossy</a>; use strings to transfer numbers via JSON.
     *
    ...
    /**
     * Returns the value mapped by {@code name} if it exists and is a long or can be coerced to a
     * long, or 0 otherwise. Note that JSON represents numbers as doubles, so this is <a
     * href="#lossy">lossy</a>; use strings to transfer numbers via JSON.
     */
    ...
    /**
     * Returns the value mapped by {@code name} if it exists, coercing it if necessary, or {@code
    ...
    /**
     * Returns the value mapped by {@code name} if it exists and is a {@code JSONArray}, or throws
     * otherwise.
     *
    ...
    /**
     * Returns the value mapped by {@code name} if it exists and is a {@code JSONArray}, or null
     * otherwise.
     */
    ...
    /**
     * Returns the value mapped by {@code name} if it exists and is a {@code JSONObject}, or throws
     * otherwise.
     *
    ...
    /**
     * Returns the value mapped by {@code name} if it exists and is a {@code JSONObject}, or null
     * otherwise.
     */
