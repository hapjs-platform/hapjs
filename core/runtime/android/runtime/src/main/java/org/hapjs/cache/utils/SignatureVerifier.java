...
public class SignatureVerifier {
    ...
    private static final int SMALL_CHUNK_SIZE_BYTES = 1024 * 1024;
    private static final int CHUNK_SIZE_BYTES = Integer.MAX_VALUE;
    ...
    private static final long SIG_BLOCK_MAGIC_HI = 0x3234206b636f6c42L;
    // the apk sign magic for debug
    // private static final long SIG_BLOCK_MAGIC_LO = 0x20676953204b5041L;
    private static final long SIG_BLOCK_MAGIC_LO = 0x20676953204b5052L;
    private static final int SIG_BLOCK_MIN_SIZE = 32;
    // ====the following 2 block ids are used to verify packages that have not been split
    private static final int BLOCK_ID_SIGNATURE = 0x01000101;
    // block id for split package to 1MB chunk to compute digest
    private static final int BLOCK_ID_SIGNATURE_SMALL_CHUNK = 0x01000102;
    // ====the following 3 block ids are used to verify packages that have been split
    private static final int BLOCK_ID_SIGNATURE_FILE_LIST = 0x01000201;
    private static final int BLOCK_ID_PLATFORM_SIGNATURE = 0x02000101;
    // block id for split package to 1MB chunk to compute digest
    private static final int BLOCK_ID_PLATFORM_SIGNATURE_SMALL_CHUNK = 0x02000102;
    // the following block id is used to verify packages that have been verified by platform
    private static final int BLOCK_ID_PLATFORM_SIGNATURE_MANIFEST = 0x02000201;
    ...
    public static X509Certificate[][] verify(String apkFile)
        ...
        try (RandomAccessFile apk = new RandomAccessFile(apkFile, "r")) {
            SigningBlockInfo signingBlockInfo = findSigningBlock(apk);
            SignatureInfo signatureInfo = signingBlockInfo.getSignatureInfo();
            if (signatureInfo != null) {
                X509Certificate[][] certificates = verify(apk, signatureInfo);
                return certificates;
            }

            SignatureInfo platformSignatureInfo = signingBlockInfo.getPlatformSignatureInfo();
            if (platformSignatureInfo != null) {
                X509Certificate[][] certificates =
                        verifyPlatformSignature(apk, platformSignatureInfo);
                SignatureInfo fileListSignatureInfo = signingBlockInfo.getFileListSignatureInfo();
                if (fileListSignatureInfo == null) {
                    throw new SecurityException("Split package has no file list signature!");
                }
                FileListSignatureVerifier.verify(
                        new File(apkFile), fileListSignatureInfo, certificates[0][0]);
                return certificates;
            }
    ...
    private static X509Certificate[][] verify(
            RandomAccessFile apkFileDescriptor, SignatureInfo signatureInfo)
        ...
        if (!signatureInfo.skipDigestVerify) {
            ...
            verifyIntegrity(
                    contentDigests,
                    apkFileDescriptor,
                    signatureInfo.signingBlockInfo.apkSigningBlockOffset,
                    signatureInfo.signingBlockInfo.centralDirOffset,
                    signatureInfo.signingBlockInfo.eocdOffset,
                    signatureInfo.signingBlockInfo.eocd,
                    signatureInfo.smallChunk);
        }
    ...
    /**
     * Returns the APK Signature Scheme v2 block contained in the provided APK file and the additional
     * information relevant for verifying the block against the file.
     *
     * @throws SignatureNotFoundException if the APK is not signed using APK Signature Scheme v2.
     * @throws IOException                if an I/O error occurs while reading the APK file.
     */
    private static SigningBlockInfo findSigningBlock(RandomAccessFile apk)
        ...
        Map<Integer, ByteBuffer> pairs = findKeyValuePairs(apkSigningBlock);

        return new SigningBlockInfo(pairs, apkSigningBlockOffset, centralDirOffset, eocdOffset,
                eocd);
    }

    /**
     * Verifies the contents of the provided APK file using builtin platform certificate.
     *
     * @param apkFileDescriptor
     * @param signatureInfo
     * @return the certificate for file list signature.
     * @throws SecurityException
     */
    private static X509Certificate[][] verifyPlatformSignature(
            RandomAccessFile apkFileDescriptor, SignatureInfo signatureInfo)
            throws SecurityException {
    ...
    private static X509Certificate[] verifySigner(
            ByteBuffer signerBlock, Map<Integer, byte[]> contentDigests,
            CertificateFactory certFactory)
        ...
        while (signatures.hasRemaining()) {
            ...
            try {
                ...
                throw new SecurityException("Failed to parse signature record #" + signatureCount,
                        e);
            }
        }
        if (bestSigAlgorithm == -1) {
    ...
    private static void verifyIntegrity(
            ...
            RandomAccessFile apkFileDescriptor,
    ...
    private static byte[][] computeContentDigests(
            int[] digestAlgorithms, DataSource[] contents, boolean smallChunk)
        ...
        int chunkSizeBytes = smallChunk ? SMALL_CHUNK_SIZE_BYTES : CHUNK_SIZE_BYTES;

        long totalChunkCountLong = 0;
        for (DataSource input : contents) {
            totalChunkCountLong += getChunkCount(input.size(), chunkSizeBytes);
        }
        ...
        for (int i = 0; i < digestAlgorithms.length; i++) {
            ...
            setUnsignedInt32LittleEndian(totalChunkCount, concatenationOfChunkCountAndChunkDigests,
                    1);
        ...
        for (DataSource input : contents) {
            ...
            while (inputRemaining > 0) {
                int chunkSize = (int) Math.min(inputRemaining, chunkSizeBytes);
    ...
    private static final long getChunkCount(long inputSizeBytes, int chunkSizeBytes) {
        return (inputSizeBytes + chunkSizeBytes - 1) / chunkSizeBytes;
    }
    ...
    static int getContentDigestAlgorithmOutputSizeBytes(int digestAlgorithm) {
        switch (digestAlgorithm) {
            case CONTENT_DIGEST_CHUNKED_SHA256:
                return 256 / 8;
            case CONTENT_DIGEST_CHUNKED_SHA512:
    ...
    static Pair<String, ? extends AlgorithmParameterSpec> getSignatureAlgorithmJcaSignatureAlgorithm(
        ...
        switch (sigAlgorithm) {
            case SIGNATURE_RSA_PSS_WITH_SHA256:
                return Pair.create(
                        "SHA256withRSA/PSS",
                        new PSSParameterSpec("SHA-256", "MGF1", MGF1ParameterSpec.SHA256, 256 / 8,
                                1));
            case SIGNATURE_RSA_PSS_WITH_SHA512:
                return Pair.create(
                        "SHA512withRSA/PSS",
                        new PSSParameterSpec("SHA-512", "MGF1", MGF1ParameterSpec.SHA512, 512 / 8,
                                1));
            case SIGNATURE_RSA_PKCS1_V1_5_WITH_SHA256:
    ...
    /**
     * Returns new byte buffer whose content is a shared subsequence of this buffer's content between
     * the specified start (inclusive) and end (exclusive) positions. As opposed to {@link
     * ByteBuffer#slice()}, the returned buffer's byte order is the same as the source buffer's byte
     * order.
     */
    ...
    /**
     * Relative <em>get</em> method for reading {@code size} number of bytes from the current position
     * of this buffer.
     *
     * <p>This method reads the next {@code size} bytes at this buffer's current position, returning
     * them as a {@code ByteBuffer} with start set to 0, limit and capacity set to {@code size}, byte
     * order set to this buffer's byte order; and then increments the position by {@code size}.
     */
    ...
    static byte[] readShortLengthPrefixedByteArray(ByteBuffer buf) throws IOException {
    ...
    private static Pair<ByteBuffer, Long> findApkSigningBlock(
        ...
        if (centralDirOffset < SIG_BLOCK_MIN_SIZE) {
            throw new SignatureNotFoundException(
        ...
        if ((footer.getLong(8) != SIG_BLOCK_MAGIC_LO)
                || (footer.getLong(16) != SIG_BLOCK_MAGIC_HI)) {
            throw new SignatureNotFoundException(
    ...
    private static Map<Integer, ByteBuffer> findKeyValuePairs(ByteBuffer apkSigningBlock)
            throws SignatureNotFoundException {
        ...
        Map<Integer, ByteBuffer> pairsMap = new HashMap<Integer, ByteBuffer>();
        int entryCount = 0;
        while (pairs.hasRemaining()) {
            ...
            if ((lenLong < 4) || (lenLong > Integer.MAX_VALUE)) {
                throw new SignatureNotFoundException(
                        "APK Signing Block entry #" + entryCount + " size out of range: "
                                + lenLong);
            }
            int len = (int) lenLong;
            ...
            // }
            pairsMap.put(id, getByteBuffer(pairs, len - 4));
    ...
    static class SigningBlockInfo {
        ...
        private Map<Integer, ByteBuffer> keyValuePairs;

        public SigningBlockInfo(
                Map<Integer, ByteBuffer> keyValuePairs,
                long apkSigningBlockOffset,
                long centralDirOffset,
                long eocdOffset,
                ByteBuffer eocd) {
        ...
        private SignatureInfo getSignatureInfo() {
            ...
            int id = BLOCK_ID_SIGNATURE_SMALL_CHUNK;
            ByteBuffer result = keyValuePairs.get(id);
            if (result == null) {
                id = BLOCK_ID_SIGNATURE;
                result = keyValuePairs.get(id);
            }
            if (result != null) {
                return new SignatureInfo(result, this, id == BLOCK_ID_SIGNATURE_SMALL_CHUNK);
            } else {
                return null;
            }
        }

        private SignatureInfo getFileListSignatureInfo() {
            int id = BLOCK_ID_SIGNATURE_FILE_LIST;
            ByteBuffer result = keyValuePairs.get(id);
            if (result != null) {
                return new SignatureInfo(result, this, false, false);
            } else {
                return null;
            }
        }

        private SignatureInfo getPlatformSignatureInfo() {
            // TODO implement
            int id = BLOCK_ID_PLATFORM_SIGNATURE_SMALL_CHUNK;
            ByteBuffer result = keyValuePairs.get(id);
            if (result == null) {
                id = BLOCK_ID_PLATFORM_SIGNATURE;
                result = keyValuePairs.get(id);
            }
            if (result != null) {
                return new SignatureInfo(result, this,
                        id == BLOCK_ID_PLATFORM_SIGNATURE_SMALL_CHUNK);
            } else {
                return null;
            }
        }
    ...
    static class SignatureInfo {
         ...
         */
        final ByteBuffer signatureBlock;

        final SigningBlockInfo signingBlockInfo;

        boolean smallChunk;

        boolean skipDigestVerify;

        private SignatureInfo(
                ByteBuffer signatureBlock,
                SigningBlockInfo signingBlockInfo,
                boolean smallChunk,
                boolean skipDigestVerify) {
            this.signatureBlock = signatureBlock;
            this.signingBlockInfo = signingBlockInfo;
            this.smallChunk = smallChunk;
            this.skipDigestVerify = skipDigestVerify;
        }

        private SignatureInfo(
                ByteBuffer signatureBlock, SigningBlockInfo signingBlockInfo, boolean smallChunk) {
            this.signatureBlock = signatureBlock;
            this.signingBlockInfo = signingBlockInfo;
            this.smallChunk = smallChunk;
            this.skipDigestVerify = false;
        }
    }
    ...
    /**
     * {@link DataSource} which provides data from a file descriptor by memory-mapping the sections of
     * the file requested by {@link DataSource#feedIntoMessageDigests(MessageDigest[], long, int)
     * feedIntoMessageDigests}.
     */
    private static final class MemoryMappedFileDataSource implements DataSource {
        ...
        private final RandomAccessFile mFd;
         ...
         * @param size     size (in bytes) of the region.
         */
        public MemoryMappedFileDataSource(RandomAccessFile fd, long position, long size) {
            mFd = fd;
        ...
        public void feedIntoMessageDigests(MessageDigest[] mds, long offset, int size)
            ...
            try {
                ...
                //         );
                FileChannel fileChannel = mFd.getChannel();
                MappedByteBuffer buf =
                        fileChannel.map(FileChannel.MapMode.READ_ONLY, filePosition, size);
    ...
    private static final class ByteBufferDataSource implements DataSource {
        /**
         * Underlying buffer. The data is stored between position 0 and the buffer's capacity. The
         * buffer's position is 0 and limit is equal to capacity.
         */
    ...
    /**
     * For legacy reasons we need to return exactly the original encoded certificate bytes, instead of
     * letting the underlying implementation have a shot at re-encoding the data.
     */
