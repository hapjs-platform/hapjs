...
describe('框架：05.debug模式下页面ViewModel数据未声明给出提示', () => {
  ...
  it('数据在data/private/protected/public中未定义', async () => {
    ...
    expect(warnMessageList[2]).equal('请确认VM的data/public/protected/private中定义了属性：b')
    expect(warnMessageList[3]).equal('请确认VM的data/public/protected/private中定义了属性：b')
    ...
    // {{}}指令
    expect(warnMessageList[0]).equal('请确认VM的data/public/protected/private中定义了属性：d')
    warnMessageList.splice(0)
    ...
    expect(warnMessageList.length).equal(1)
    expect(warnMessageList[0]).equal('请确认VM的data/public/protected/private中定义了属性：b')
    warnMessageList.splice(0)
